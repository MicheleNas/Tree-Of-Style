...
def get_gpt_response(messages: list, model="gpt-4o-mini"):
    response = client_openai.chat.completions.create(
        model=model,  # Specifica il modello da utilizzare
        messages=messages,
...
def chat_with_chatgpt4o_mini(conversation, model):
    response = get_gpt_response(conversation, model)
...
def generate_and_verify(conversation, style_embeddings, model=None):
    ...
    if model in ["llama-3.3-70b-versatile", "mixtral-8x7b-32768"]:
        ...
        response = chat_with_chatgpt4o_mini(conversation, model)
...
def alterego(user_input, conversation, style_embeddings, model, ToS=True):
    ...
    if ToS:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            response = max(results, key=lambda x: x['similarity'])
            conversation.append({"role": "assistant", "content": response['sentence']}) 
    else:
        response = generate_and_verify(conversation, style_embeddings, model)
        conversation.append({"role": "assistant", "content": response['sentence']}) 
...
# Prompt configurazione iniziale
with open('./utils/prompt_whitout_style-analisys.txt', 'r') as file:
    prompt_wsa = file.read().strip()
...
prompts = ["Zero-Shot"] # Se si usa quella senza l'analisi dello stile vuol dire che si sta testando sono ToS, con il prompt normale stiamo testando la Zero-Shot
...
for model in models:
    for prompt in prompts:
        for item in tqdm(data_test, desc=f"Elaborazione {model} con {prompt}", unit="item"):
            ...
            if prompt == "Zero-Shot":
                ...
                conversation = [{"role": "system", "content": prompt_wsa}]
                ToS=True
            else:
                conversation = [{"role": "system", "content": prompt}]
            ...
            if status:
                response, _ = alterego(f"{user_input}. Status di Irene: {status}. Rispondi tenendo in considerazione lo status. Dai solo la risposta di Irene secondo il suo stile.", conversation, style_embeddings, model, ToS)
            else:
                response, _ = alterego(f"{user_input}. Dai solo la risposta di Irene secondo il suo stile.", conversation, style_embeddings, model, ToS)
            ...
            if model in ["llama-3.3-70b-versatile","mixtral-8x7b-32768"] and prompt == "Tos":
                # Pausa tra le richieste (se vuoi aggiungere un ritardo)

                time.sleep(60)

        # Salva il risultato per il modello corrente in un file JSON
        output_path = f'test/finetuning_gpt/results/{model}_{prompt}_results.json'
